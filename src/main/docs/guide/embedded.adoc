...
Now make a request to `http://localhost:8080/api/product/1`:
...
----
{
    ...
    _embedded: {
        category: { //<1>
            _links: {
                self: {
                    href: "http://localhost:8080/api/category/1",
                    hreflang: "en_US",
                    type: "application/hal+json"
                }
            },
...
----
<1> The `category` object includes the default JSON renderer output for our `Category` resource, as well as a `_self` link so clients can request the category directly if needed.
...
fetch("http://localhost:8080/api/order/1").then(function(response) {
...
fetch("http://localhost:8080/api/product/1").then(function(response) {
    return response.json();
}).then(function(data) {
    this.product = data;
});
