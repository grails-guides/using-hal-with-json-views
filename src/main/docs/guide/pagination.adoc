...
Finally, we need to edit our `UrlMappings` to create the rest endpoints that were formerly generated using the `@Resource` annotation. Grails supports a `resource` property on URL mappings that will generate these URLs automatically. Edit `grails-app/controllers/UrlMappings.groovy` and add the following rule to the `mappings` block:

[source,groovy]
.grails-app/controller/UrlMappings.groovy
----
include::{sourceDir}/grails-app/controllers/UrlMappings.groovy[indent=0,lines="19..21"]
----
...
Make a request to `http://locahost:8080/api/product`:
