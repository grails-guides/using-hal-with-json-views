...
Make a request to `http://localhost:8080/api/order/1`, and you should see `_links` for each product in the `products` array:
  ...
  products: [
    {
      _links: {
        self: {
          href: "http://localhost:8080/api/product/11",
          hreflang: "en_US",
    ...
    },
    {
      _links: {
        self: {
          href: "http://localhost:8080/api/product/6",
          hreflang: "en_US",
          type: "application/hal+json"
        }
      },
